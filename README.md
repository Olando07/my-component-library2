...
`docker build -t McDonald_Olando_coding_assignment13 .`
...
2. Run the Container
`docker run -d -p 8018:8018 --name McDonald_Olando_coding_assignment13 McDonald_Olando_coding_assignment13`
Command breakdown:

`-d` → Run in detached mode (background)
`-p 8018:8018` → Map container port 8083 to localhost port 8083
`--name McDonald_Olando_coding_assignment13` → Name the container
